...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    onlyOwner
  {
    ...
    whitelistedNFTContractsToIndex[_contractAddress] = whitelistedNFTContracts
      .length;
  }
  ...
  * This is a reason to not execute a delete nftContractToFee[_contractAddress]
    ...
    onlyOwner
  {
    ...
    uint256 contractIndex = whitelistedNFTContractsToIndex[_contractAddress] -
      1;
  ...
  */
  function getwhitelistedNFTContracts() public view returns (address[] memory) {
  ...
  function setFee(address _NFTContract, uint256 _fee) public onlyOwner {
    require(_NFTContract != address(0), "Error: Can't set fee for address 0");
  ...
  function getFee(address _NFTContract) public view returns (uint256) {
    require(_NFTContract != address(0), "Can't retrieve fee from address 0");
  ...
  */
  ...
  {
    require(_NFTContract != address(0), "Contract address can't be address(0)");
    ...
    emit MarketItemCreated(itemId, _tokenId, floorPrice, _NFTContract, owner());
