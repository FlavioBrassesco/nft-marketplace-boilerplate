...
describe("NFTAuctions", () => {
  ...
  describe("createAuctionItem", () => {
    it("Should revert if panic switch true", async () => {
      ...
      await expect(
        nftauctions
          .connect(addr1)
          .createAuctionItem(nftminter.address, 0, eth$("0.1"), 1)
    ...
    it("Should revert if addr1 tries to create an AuctionItem with floor price 0", async () => {
      ...
      await expect(
        nftauctions.connect(addr1).createAuctionItem(nftminter.address, 0, 0, 1)
    ...
    it("Should revert if addr1 tries to create an AuctionItem with days out of bounds", async () => {
      ...
      await expect(
        nftauctions
          .connect(addr1)
          .createAuctionItem(nftminter.address, 0, eth$("0.1"), 0)
      ).to.be.revertedWith("Duration out of bounds");

      await expect(
        nftauctions
          .connect(addr1)
          .createAuctionItem(nftminter.address, 0, eth$("0.1"), 8)
      ).to.be.revertedWith("Duration out of bounds");
    });
    ...
    it("Should pass if addr1 successfully creates an AuctionItem", async () => {
      ...
      expect(auctionItem.currentBidder).to.equal(ADDR_0);
  ...
  describe("bid", () => {
    it("Should revert if panic switch true", async () => {
      ...
      await expect(
        nftauctions.connect(addr1).bid(nftminter.address, 0, ADDR_0, eth$("0.1"), { value: eth$("0.1") })
      ).to.be.revertedWith("Pausable: paused");
    });
    ...
    it("Should revert if addr1 tries to create a bid for its own item", async () => {
      ...
      const txCreateAuction = await nftauctions
        .connect(addr1)
        .createAuctionItem(nftminter.address, 0, price, 1);
      txCreateAuction.wait();

      const options = { value: price };

      await expect(
        nftauctions.connect(addr1).bid(nftminter.address, 0, ADDR_0, price, options)
    ...
    it("Should revert if addr2 tries to create a second bid for addr1 item", async () => {
      ...
      const txbid = await nftauctions
        .connect(addr2)
        .bid(nftminter.address, 0, ADDR_0, price, options);
      txbid.wait();

      const options2 = { value: price };
      await expect(
        nftauctions.connect(addr2).bid(nftminter.address, 0, ADDR_0, price, options2)
    ...
    it("Should revert if addr2 tries to create a bid when addr1 auction has finished", async () => {
      ...
      const options = { value: price };
      await expect(
        nftauctions.connect(addr2).bid(nftminter.address, 0, ADDR_0, price, options)
    ...
    it("Should revert if addr2 tries to create a bid lower than floor price", async () => {
      ...
      await expect(
        nftauctions.connect(addr2).bid(nftminter.address, 0, ADDR_0, price.sub(1), options)
    ...
    it("Should revert if addr3 tries to bid lower than addr2", async () => {
      ...
      const txBid1 = await nftauctions
        .connect(addr2)
        .bid(nftminter.address, 0, ADDR_0, price.mul(4), options);
      txBid1.wait();

      const options2 = { value: price.div(2) };
      await expect(
        nftauctions.connect(addr3).bid(nftminter.address, 0, ADDR_0, price.div(2), options2)
  ...
  describe("AuctionItem Enumeration", () => {
    it("Should pass if enumeration of all AuctionItems for addr1 is ok", async () => {
      ...
      for (let i = 0; i < qty; i++) {
        const txCreate = await nftauctions
          .connect(addr1)
          .createAuctionItem(nftminter.address, i, price.add(i), i + 1);
        txCreate.wait();
        timestamps[i] = (
    ...
    it("Should pass if enumeration of all AuctionItems is ok", async () => {
      ...
      for (let i = 2; i < qty + 2; i++) {
        const txCreate = await nftauctions
          .connect(addr2)
          .createAuctionItem(nftminter.address, i, price2.add(i), i + 1);
        txCreate.wait();
        timestamps[i] = (
          await ethers.provider.getBlock(txCreate.blockNumber)
        ).timestamp;
